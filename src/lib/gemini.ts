...
function resetModel(failedModel: string): void {
  ...
  if (idx >= 0 && idx < models.length - 1) currentModel = models[idx + 1];
}
  ...
  for (let attempt = 0; attempt < models.length; attempt++) {
    try {
      ...
      const chat = genModel.startChat({
        ...
        systemInstruction: {
          role: "system",
          parts: [{ text: systemInstruction || "You are EduPro AI, a helpful educational assistant." }],
        },
      });
...
export function createChatHistory(messages: Array<{ role: string; content: string }>) {
