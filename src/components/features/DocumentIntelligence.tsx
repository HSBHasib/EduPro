...
export function DocumentIntelligence() {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.type === "application/pdf") {
      ...
      try {
        ...
        addToast({ title: `Extracted text from ${file.name}`, color: "success" });
      } catch {
        addToast({ title: "Failed to extract text from PDF", color: "danger" });
      } finally {
        setExtracting(false);
      }
      ...
      setUploadedFile(file.name);
      addToast({ title: `Loaded ${file.name}`, color: "success" });
    } else {
      addToast({ title: "Please upload a PDF or TXT file", color: "danger" });
    }
  ...
  async function onSubmit(data: DocumentAnalysisInput) {
    ...
    try {
      ...
      setResult(res.data);
      addToast({ title: "Document analyzed successfully!", color: "success" });
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Failed to analyze document";
      if (msg.includes("429") || msg.includes("quota")) {
        addToast({ title: "AI service quota exceeded. Please try again later or check your API key.", color: "danger" });
      } else {
        addToast({ title: msg.slice(0, 120), color: "danger" });
      }
    } finally {
      setLoading(false);
    }
  ...
  function downloadPDF() {
    ...
    addToast({ title: "PDF downloaded!", color: "success" });
  }
